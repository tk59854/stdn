...
from utils import get_valid_args, is_list, is_dict, np2var, ts2np, list2var, get_attr_from, EMA, test_imgs
...
class MetaModel(metaclass=ABCMeta):
    ...
    @abstractmethod
    def save_ema(self, ema, iteration):
        pass
...
class BaseModel(MetaModel, nn.Module):
    ...
    def __init__(self, cfgs, training):
        ...
        self.save_path = osp.join('output/', cfgs['data_cfg']['dataset_name'],
                                  cfgs['model_cfg']['model'], self.cfgs['save_name'])

        self.build_network(cfgs['model_cfg'])
        self.init_parameters()

        self.msg_mgr.log_info('------- Data cfg -------')
        self.msg_mgr.log_info(cfgs['data_cfg'])
        self.msg_mgr.log_info('------- Loss cfg -------')
    ...
    def get_optimizer(self, optimizer_cfg):
        ...
        tune_ids = list()
        tune_module = optimizer_cfg['tune_module']
        for _module in self.named_children():
            if _module[0] in tune_module:
                tune_ids += list(map(id, _module[1].parameters()))

        base_params = filter(lambda p: p.requires_grad and id(p) not in tune_ids, self.parameters())
        tune_params = filter(lambda p: p.requires_grad and id(p) in tune_ids, self.parameters())
        optimizer = optimizer([
            {'params': base_params},
            {'params': tune_params, 'lr': optimizer_cfg['lr']*optimizer_cfg['tune_scale']}
        ], **valid_arg)
        return optimizer

    def get_scheduler(self, scheduler_cfg):
        if scheduler_cfg['scheduler'] == 'CosWithWarmUp':
            warm_up_step = int(self.engine_cfg['total_iter'] * 0.08)
            iters = self.engine_cfg['total_iter'] - warm_up_step
            floor = self.cfgs['scheduler_cfg']['min_lr'] / self.cfgs['optimizer_cfg']['lr']
            _lambda = lambda epoch: epoch * 1. / warm_up_step if epoch <= warm_up_step else \
                floor + 0.5 * (1. - floor) * (1 + np.cos(np.pi * (epoch - warm_up_step + 1)/iters))
            scheduler = torch.optim.lr_scheduler.LambdaLR(self.optimizer, _lambda)

        elif scheduler_cfg['scheduler'] == 'MultiStepWarmUp':
            warm_up_step = int(self.engine_cfg['total_iter'] * 0.08)
            milestones = self.cfgs['scheduler_cfg']['milestones']
            floor = 0.1 ** len(milestones)
            gamma = self.cfgs['scheduler_cfg']['gamma']
            _lambda = lambda epoch: (1. - floor) * epoch / warm_up_step + floor if epoch < warm_up_step else \
                gamma ** bisect_right(milestones, epoch)
            scheduler = torch.optim.lr_scheduler.LambdaLR(self.optimizer, _lambda)
        else:
    ...
    def resume_ckpt(self, restore_hint):
        if isinstance(restore_hint, int):
            ...
            save_name = self.cfgs['save_name']
            save_name = osp.join(
                self.save_path, 'checkpoints/{}-{}.pt'.format(save_name, restore_hint))
            self.iteration = restore_hint.split('-')[0]
        else:
            raise ValueError(
    ...
    def ema(self, model):
        if not self.cfgs['evaluator_cfg']['ema']:
            return None
        ema = EMA(model, 0.999)
        ema.register()
        self.msg_mgr.log_info('EMA registered!')
        return ema

    def save_ema(self, ema, iteration):
        ema.apply_shadow()
        shadow = {'model': self.state_dict()}
        save_name = self.cfgs['save_name']
        torch.save(shadow,
                   osp.join(self.save_path, 'checkpoints/{}-{:0>5}-ema.pt'.format(save_name, iteration)))
        ema.restore()
    ...
    def run_train(model):
        ...
        for inputs in model.train_loader:
            ...
            if model.iteration % model.engine_cfg['save_iter'] == 0:
                ...
                if ema is not None:
                    model.save_ema(ema, model.iteration)
    ...
    def toy_test(dir_path, model):
        test_imgs(dir_path, model)
