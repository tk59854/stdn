...
class TripletLoss(BaseLoss):
    ...
    def forward(self, embeddings, labels):
        ...
        mean_ap = ap_dist.sum() / (ap_dist != 0).sum().float()
        mean_an = an_dist.sum() / (an_dist != 0).sum().float()
        # hard
        hard_sample = F.relu(ap_dist - an_dist)
        num_hard = (hard_sample != 0).sum().float()
        mean_hard = hard_sample.sum() / num_hard
        # eazy_simi - eazy
        num_simi = (F.relu(an_dist - ap_dist) != 0).sum().float() - (F.relu(an_dist - ap_dist - self.margin) != 0).sum().float()
        ...
        self.info.update({
            ...
            'mean_ap': mean_ap.detach().clone(),
            'mean_an': mean_an.detach().clone(),
            'num_hard': num_hard.detach().clone(),
            'num_simi': num_simi.detach().clone(),
            'mean_hard': mean_hard.detach().clone()})
