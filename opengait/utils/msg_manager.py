...
class MessageManager:
    ...
    def init_manager(self, save_path, log_to_file, log_iter, total_iter, iteration=0):
    ...
    def write_to_tensorboard(self, summary):
        ...
        for k, v in summary.items():
            ...
            v = vutils.make_grid(
                v, normalize=True, scale_each=True, padding=2, pad_value=0) if 'image' in module_name else v
    ...
    def log_training_info(self):
        ...
        iter_time = (now - self.time) / self.log_iter
        eta = (self.total_iter - self.iteration) * iter_time

        t_m, t_s = divmod(eta, 60)
        t_h, t_m = divmod(t_m, 60)
        all_time = '{:02d}:{:02d}:{:02d}'.format(int(t_h), int(t_m), int(t_s))

        string = "Iteration {:0>5}, Cost {:.2f}s, ETA {}".format(
            self.iteration, now-self.time, all_time, end="")
