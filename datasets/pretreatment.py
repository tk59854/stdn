...
def pretreat(input_path: Path, output_path: Path, img_size: int = 64, workers: int = 4, verbose: bool = False, dataset: str = 'CASIAB') -> None:
    ...
    with mp.Pool(workers) as pool:
        for _subdict, _total_frames in pool.map(partial(listing, input_path=input_path), all_id):
            logger.info('--- {}/{} listed'.format(i, len(all_id)))
            i += 1
            img_groups = {**img_groups, **_subdict}
            all_frames += _total_frames
    logging.info('Total files listed : {}'.format(all_frames))
    logging.info('List Done')
...
def listing(_id, input_path):
    sub_dict = defaultdict(list)
    total_files = 0
    _path = Path(os.path.join(input_path, _id))
    for img_path in _path.rglob('*.png'):
        if 'gei.png' in img_path.as_posix():
            continue
        *_, sid, seq, view, _ = img_path.as_posix().split('/')
        sub_dict[(sid, seq, view)].append(img_path)
        total_files += 1
    logging.info('--- total files listed for id {}: {}'.format(_id, total_files))
    
    return sub_dict, total_files
